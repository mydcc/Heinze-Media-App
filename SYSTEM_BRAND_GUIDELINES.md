# HEINZE MEDIA - System Brand Guidelines
...
HEINZE MEDIA steht für **moderne, performante Web-Technologien** mit Fokus auf **XR/AR/VR** und **immersive Experiences**. Das Design System reflektiert diese Vision durch:

- **4 thematische Welten** (Purple Meteorite, Steel Pro, Ever Free, Insight)
- **Dark/Light Mode** Support für alle Themes
- **Bleeding-Edge Tech**: Svelte 5 Runes, Tailwind CSS v4, Vite 7
- **SEO-First**: Lighthouse 100/100 als Standard
- **Berlin-Based**: DSGVO-Compliance, deutsche & englische Sprachunterstützung
...
<script lang="ts">
  ...
  let { title, description, image, href, theme = 'meteorite' }: Props = $props();
</script>

<article class="card theme-{theme}">
  {#if image}
...
<nav class="main-nav">
  <div class="container">
    <a href="/" class="logo">
      <img src="/logo.png" alt="HEINZE MEDIA" />
    </a>
    ...
    <button onclick={() => themeState.toggleMode()}>
...
<script lang="ts">
  ...
  let { title, subtitle, ctaText = 'Get Started', ctaHref = '/contact' }: Props = $props();
</script>
...
<style>
  ...
  .hero-title {
    font-size: clamp(3rem, 8vw, var(--text-h1));
    margin-bottom: 1rem;
    background: linear-gradient(135deg, var(--text-main), var(--highlight-primary));
...
type ThemeName = 'meteorite' | 'steel' | 'ever' | 'insight';
type Mode = 'dark' | 'light';

class ThemeState {
  theme = $state<ThemeName>('meteorite');
  mode = $state<Mode>('dark');

  // Computed
  themeClass = $derived(`theme-${this.theme}`);
...
<script lang="ts">
  ...
  // Computed mit $derived
  let displayTitle = $derived(isActive ? title.toUpperCase() : title);

  // Effects mit $effect
  $effect(() => {
    console.log(`Title changed: ${displayTitle}`);
  });
</script>
...
- Purple `#4e21e7` auf Dark `#0c082f`: ✅ 4.8:1
- White `#ffffff` auf Purple: ✅ 7.2:1
- Text-Muted auf Backgrounds: ✅ 4.6:1
...
button:focus-visible, a:focus-visible {
  box-shadow: 0 0 0 3px rgba(78, 33, 231, 0.3);
}
```
...
<!-- ARIA Labels -->
<button aria-label="Toggle dark mode" onclick={toggleMode}>
  🌙
</button>
...
- **Main Logo:** `/static/logo.png`
- **Favicon:** `/static/favicon.ico`
- **OG Image:** `/static/og-image.png`

### Brand Guideline PDFs

- **Vollständig:** `/brand_guidelines/brand-guidelines_page-0001.jpg` bis `page-0017.jpg`

### Pattern & Backgrounds

- **Light Pattern:** `/assets/pattern-lg.svg`
- **Gradient Meshes:** Generiert via CSS Gradients

---

## 🚀 Quick Implementation Checklist

- [ ] app.css mit allen 4 Themes einrichten
- [ ] Theme State (theme.svelte.ts) implementieren
- [ ] Button Component erstellen
- [ ] Card Component erstellen
- [ ] Navigation Component erstellen
- [ ] Hero Component mit Threlte-3D
- [ ] SEOHead für alle Pages
...
- **Threlte (Three.js):** <https://threlte.xyz/>
- **WCAG 2.1:** <https://www.w3.org/WAI/WCAG21/quickref/>

---

*Dieses Dokument ist ein Living Document und wird mit der Evolution der HEINZE MEDIA Design Language aktualisiert.*
